...
Change `https://teleport.example.com:3080` to the address and port of your Teleport Auth Server. If you are a Teleport Cloud cluster, use your tenant's subdomain, e.g., `mytenant.teleport.sh`. 
...
Replace `grafana` with the value of the `--app-name` flag you used when starting the Teleport Application Service and `teleport.example.com` with the address of your Auth Service or Proxy Service.
