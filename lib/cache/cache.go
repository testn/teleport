...
func (c *Cache) GetClusterConfig(opts ...services.MarshalOption) (services.ClusterConfig, error) {
	...
	defer rg.Release()
	if !rg.IsCacheRead() {
		ta := func(_ services.ClusterConfig) {} // compile-time type assertion
		ci, err := c.fnCache.Get(context.TODO(), clusterConfigCacheKey{"main"}, func() (interface{}, error) {
			cfg, err := rg.clusterConfig.GetClusterConfig(services.AddOptions(opts, services.SkipValidation())...)
			ta(cfg)
			return cfg, err
		})
		if err != nil {
		...
		cachedCfg := ci.(services.ClusterConfig)
		ta(cachedCfg)
...
func (c *Cache) GetClusterName(opts ...services.MarshalOption) (services.ClusterName, error) {
	...
	if !rg.IsCacheRead() {
		...
		ci, err := c.fnCache.Get(context.TODO(), clusterConfigCacheKey{"name"}, func() (interface{}, error) {
			cfg, err := rg.clusterConfig.GetClusterName(services.AddOptions(opts, services.SkipValidation())...)
			ta(cfg)
			return cfg, err
		})
		if err != nil {
...
func (c *Cache) GetNodes(ctx context.Context, namespace string, opts ...services.MarshalOption) ([]services.Server, error) {
	...
	return rg.presence.GetNodes(ctx, namespace, services.AddOptions(opts, services.SkipValidation())...)
}
