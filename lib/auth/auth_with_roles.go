...
func (r *webSessionsWithRoles) Delete(ctx context.Context, req types.DeleteWebSessionRequest) error {
	if err := r.c.checkPermissionToDeleteUserWebSession(req.User); err != nil {
		return trace.Wrap(err)
	}
...
type accessChecker interface {
	...
	checkPermissionToDeleteUserWebSession(username string) error
}
...
func (a *ServerWithRoles) DeleteAppSession(ctx context.Context, req types.DeleteAppSessionRequest) error {
	...
	if err := a.checkPermissionToDeleteUserWebSession(session.GetUser()); err != nil {
		return trace.Wrap(err)
	}
...
func (a *ServerWithRoles) DeleteUserAppSessions(ctx context.Context, req *proto.DeleteUserAppSessionsRequest) error {
	...
	if err := a.checkPermissionToDeleteUserWebSession(req.Username); err != nil {
		return trace.Wrap(err)
	}
	...
	for _, session := range sessions {
		// The check is done against each session to avoid comparing user input
		// with resources that the user doesn’t have access to.
		if err := a.checkPermissionToDeleteUserWebSession(session.GetUser()); err != nil {
			if trace.IsAccessDenied(err) {
				continue
			}
		...
		// A request will only delete web sessions from the `Username` provided.
		if req.Username != session.GetUser() {
			continue
		}

		deleteRequest := types.DeleteAppSessionRequest{SessionID: session.GetName()}
		if err := a.authServer.DeleteAppSession(ctx, deleteRequest); err != nil {
			return trace.Wrap(err)
		}
	}
...
// checkPermissionToDeleteUserWebSession checks if the current user can delete
// WebSessions from the provided `username`.
func (a *ServerWithRoles) checkPermissionToDeleteUserWebSession(username string) error {
