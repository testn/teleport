...
func TestTokensCRUD(t *testing.T) {
	...
	require.Error(t, err)
	require.Equal(t,
		`node "bad-node-name" [bad-host-id] can not join the cluster, the token does not allow "Proxy" role`,
		err.Error())
	...
	require.NoError(t, err)
	require.True(t, apiutils.SliceContainsStr(hostCert.ValidPrincipals, "example.com"),
		"can't find example.com in %v", hostCert.ValidPrincipals)
	...
	require.Equal(t, `"node-name" [late.bird] can not join the cluster with role Proxy, the token is not valid`, err.Error())
