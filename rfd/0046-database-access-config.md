...
```bash
teleport db configure create [--auth-server] [--token=] [--discovery-rds=] [--discovery-redshift] [--output=] [--ca-pin=] [--name=] [--protocol=] [--uri=] [--labels=]
```
...
```bash
$ teleport db configure bootstrap [-c=,--config=] [--manual] [--policy-name=] [--attach-to=] [--confirm]
```
...
$ teleport db configure bootstrap --attach-to SampleUser --confirm
Reading configuration at "/etc/teleport.yaml"...
...
$ teleport db configure bootstrap --manual --attach-to SampleUser
Reading configuration at "/etc/teleport.yaml"...
